  ...
  return (
    <div className="av-players">
      <div className="av-players-head">
        <span>Oyuncular</span><span className="tnum">{players.length}</span>
      </div>
      {players.length === 0 ? (
        <div className="av-players-empty">Bu tur için henüz bahis yok</div>
      ) : (
        <div className="av-players-list">
          {sorted.map((p) => (
            <div key={p.id} className={`av-prow st-${p.status} ${p.user_id === meId ? 'is-me' : ''}`}>
              <span className="av-pname">{p.user_id === meId ? 'Sen' : p.username}{p.slot === 2 ? ' ·2' : ''}</span>
