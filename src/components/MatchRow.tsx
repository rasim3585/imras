...
// One compact bulletin line (Nesine layout): "Home (P) - Away (P)" on a single
// row + quick 1/X/2/O-U/BTTS odds + a "+N" that expands ALL markets inline
// (compact, Nesine-size cells).
export default function MatchRow({ match, live }: { match: Match; live?: LiveState }) {
  ...
  const shownMarkets = [...match.markets]
    .filter((m) => !(isLive && HT.has(m.market_type)))
      ...
      <div className={`ll-row ${isLive ? 'is-live' : ''}`}>
        <Link className="ll-info" to={`/match/${match.id}`}>
          ...
          <span className="ll-match">
            ...
            <span className="ll-vs tnum">{isLive ? `${live!.home_score}-${live!.away_score}` : 'v'}</span>
        ...
        <div className="ll-odds"><Cell m={mr} k="home" /><Cell m={mr} k="draw" /><Cell m={mr} k="away" /></div>
        <div className="ll-odds ll-odds-sec"><Cell m={ou} k="ou25_under" sec /><Cell m={ou} k="ou25_over" sec /><Cell m={kg} k="btts_yes" sec /></div>
